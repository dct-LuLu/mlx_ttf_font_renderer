...
static int	file_len(const char *path, size_t *total)
{
	...
	if (fd == -1)
		return (rerror(errno, ": file_len %s", path));
	while (true)
	{
		read_ret = read(fd, temp, sizeof(temp));
		if (read_ret < 0)
			return (close(fd), rerror(errno, ": file_len %s", path));
		*total += read_ret;
		if (read_ret < 4096)
			break ;
	}
...
int	load_file(const char *path, t_buffer **buf)
{
	...
	*buf = ft_calloc(sizeof(t_buffer), 1);
	if (!*buf)
		return (rerror(errno, ": load_file buf"));
	ret = file_len(path, &(*buf)->size);
	if (ret)
		return (ret);
	(*buf)->data = ft_calloc((*buf)->size, 1);
	if (!(*buf)->data)
		return (rerror(errno, ": load_file data"));
	fd = open(path, O_RDONLY);
	if (fd == -1)
	{
		ret = errno;
		return (close(fd), rerror(ret, ": load_file %s", path));
	}
	ret = read(fd, (*buf)->data, (*buf)->size);
	if ((size_t)ret != (*buf)->size)
		return (close(fd), rerror(ERR_READ_FILE, ": load_file %s %d %d", path,
				ret, (*buf)->size));
	return (0);
}
